...
//!
//! Mirrors `terraphim_agent`'s `update_refusal_tests`: the packaging
//! lifecycle gates install `terraphim-grep` via dpkg/rpm and require an
//! explicit `update` to refuse with a non-zero exit, the exact stderr line,
//! and no write to the installed executable, while `check-update` keeps
//! reporting the managed status on stdout with a zero exit.
//!
//! The real compiled binary is staged into a temporary prefix with a real
//! receipt file (`<prefix>/share/terraphim/package-manager.d/<bin-name>`),
//! using the updater's own path-derived detection: no network, no system
//! paths, nothing mocked.

use std::fs;
use std::path::PathBuf;
use std::process::Command;

const BIN_NAME: &str = "terraphim-grep";

/// Stage the real compiled binary into `<tmp>/bin/terraphim-grep` with a
...
fn stage_managed_binary(manager: &str) -> (tempfile::TempDir, PathBuf) {
    ...
    fs::copy(env!("CARGO_BIN_EXE_terraphim-grep"), &bin).expect("copy real binary");
