#!/usr/bin/env bash
# Wrong-architecture regression tests for build-client-packages.sh.
#
...
# TERRAPHIM_BUILD_CLIENT_PACKAGES_SOURCED=1.
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/terraphim-client-nfpm-arch.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
# failure so wiring it into CI without provisioning nFPM/dpkg-deb/Docker
...
run_sourced_verifier() {
    ...
    (
        ...
        export BIN_NAME=terraphim-agent
...
BINARY="$TMP/qualified/terraphim-agent"
make_fixture_binary "$BINARY" terraphim-agent
mkdir -p "$TMP/work"
...
test_wrong_arch_deb_fails_closed() {
    local wrong_deb="$TMP/terraphim-agent_9.8.7-1_arm64.deb"
...
test_wrong_arch_rpm_fails_closed() {
    local wrong_rpm="$TMP/terraphim-agent-9.8.7-1.aarch64.rpm"

    if ! command -v rpm >/dev/null 2>&1 && ! docker info >/dev/null 2>&1; then
        require_tool_or_skip "RPM arch regression needs host rpm or Docker"
        return 0
    fi
...
echo "client nFPM wrong-arch regression tests passed"
